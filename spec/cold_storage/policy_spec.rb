...
RSpec.describe ColdStorage::Policy do
  describe 'option validation' do
    it 'rejects unknown options' do
      expect { described_class.new(Note, afterr: 1.day) }
        .to raise_error(ColdStorage::InvalidPolicyError, /unknown archivable option/)
    end

    it 'refuses a policy without any criterion' do
      expect { described_class.new(Note, every: 1.month) }
        .to raise_error(ColdStorage::InvalidPolicyError, /at least one of/)
    end

    it 'rejects a non duration' do
      expect { described_class.new(Note, after: 'a while') }
        .to raise_error(ColdStorage::InvalidPolicyError, /duration/)
    end
    ...
    it 'reports an unknown column against the real table' do
      ...
      expect { policy.validate_against_schema!(Note) }
        .to raise_error(ColdStorage::InvalidPolicyError, /has no column :closed_at/)
    end
  end
