...
RSpec.describe ColdStorage::SchemaMirror do
  ...
  describe '#sync!' do
    ...
    it 'creates its own bookkeeping tables' do
      archive_connection do |connection|
        expect(connection.table_exists?('cold_storage_runs')).to be(true)
        expect(connection.table_exists?('cold_storage_metadata')).to be(true)
      end
    end
  ...
  describe 'following migrations' do
    ...
    it 'drops them when drop_removed_columns is on' do
      ...
      ColdStorage.config.drop_removed_columns = true
      described_class.new(models: [Invoice]).sync!
