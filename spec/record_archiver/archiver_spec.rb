...
RSpec.describe RecordArchiver::Archiver do
  ...
  describe 'moving rows' do
    ...
    it 'can copy without deleting' do
      Invoice.archiving_policy = RecordArchiver::Policy.new(Invoice, after: 12.months, delete_after_archive: false)
      result = Invoice.archive_now!
  ...
  describe 'the every: window' do
    ...
    it 'skips a model that ran recently' do
      ...
      result = RecordArchiver.archive(Invoice)
    ...
    it 'runs again once the window elapsed' do
      Invoice.archive_now!
      RecordArchiver::Run.for_model(Invoice).update_all(finished_at: 2.months.ago)
      create_invoice(number: 'old-2')

      expect(RecordArchiver.archive(Invoice)).not_to be_skipped
    end

    it 'records the run' do
      Invoice.archive_now!
      run = RecordArchiver::Run.for_model(Invoice).last
  ...
  describe 'guard rails' do
    it 'refuses to archive a model without a policy' do
      expect { RecordArchiver.archive(InvoiceLine) }
        .to raise_error(RecordArchiver::NotArchivableError, /not archivable/)
    end
    ...
    it 'explains what to do when the archive table is missing' do
      ...
      expect { Note.archive_now! }
        .to raise_error(RecordArchiver::SchemaMissingError, /record_archiver:schema:sync/)
    ensure
      RecordArchiver.sync_schema!(models: [Note])
    end
  end
end
