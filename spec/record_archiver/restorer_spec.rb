...
RSpec.describe RecordArchiver::Restorer do
  ...
  it 'moves rows back into the primary database' do
    expect(RecordArchiver.restore(Invoice, [invoice.id])).to eq(1)
  ...
  it 'removes them from the archive' do
    RecordArchiver.restore(Invoice, [invoice.id])
  ...
  it 'accepts an archive relation' do
    expect(RecordArchiver.restore(Invoice, Invoice.archived.where(number: 'old'))).to eq(1)
    expect(Invoice.unscoped.count).to eq(1)
  end

  it 'can keep the archived copy' do
    RecordArchiver.restore(Invoice, [invoice.id], delete_from_archive: false)
  ...
  describe 'with relations' do
    ...
    before do
      ...
      RecordArchiver::Archiver.new(Tax, relation: Tax.unscoped.all, force: true).call
      Invoice.archive_now!
    end
    ...
    it 'points at the owner when asked to restore a belongs_to' do
      expect { RecordArchiver.restore(InvoiceLine, [1], with: [:invoice]) }
